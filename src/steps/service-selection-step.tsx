...
import { formatPrice, formatDuration, formatPriceAdjustment } from '@/lib/services';
...
export function ServiceSelectionStep({
  ...
  const categorized = Object.entries(
    ...
    ([, a], [, b]) => (a[0]?.category_display_order || 999) - (b[0]?.category_display_order || 999)
  );
            ...
            >
              ...
              <div className="grid grid-cols-1 gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {categoryServices.map((service, idx) => {
                  ...
                  const options: ServiceOption[] = serviceOptionsMap[service.id] || [];
                  const chosenOption = selectedServiceOptions[service.id] ?? null;
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        transform: visibleElements.has(`service-card-${service.id}`)
                          ? 'translateY(0)'
                          : 'translateY(30px)',
                    ...
                    >
                      {/* Service header — click to select/deselect */}
                      <div
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          ...
                          {isSelected && (
                            <CheckIcon className="flex-shrink-0 ml-2 w-6 h-6 bg-indigo-600 rounded-full text-white p-1" />
                          )}
                        </div>
                        {service.description && (
                      ...
                      {/* Options panel — shown only when service is selected and it has options */}
                      {isSelected && options.length > 0 && (
                        <div className="border-t border-indigo-200 bg-white px-4 pb-4 sm:px-6 sm:pb-5 rounded-b-lg">
                          <p className="text-xs font-semibold text-indigo-700 uppercase tracking-wide mt-3 mb-2">
                            Choose an option
                          </p>
                              ...
                              >
                                <div className="flex items-center space-x-2">
                                  <input
                                    ...
                                    checked={chosenOption?.id === opt.id}
                                    onChange={() => onOptionSelect(service.id, opt)}
                                ...
                                <div className="text-right ml-3 flex-shrink-0">
                                  <span
                                    className={`text-sm font-semibold ${opt.price_adjustment > 0 ? 'text-amber-700' : opt.price_adjustment < 0 ? 'text-green-700' : 'text-gray-500'}`}
                                  >
          ...
          >
            ...
            <div className="space-y-3">
              {selectedServices.map((service) => {
                ...
                return (
                  <div key={service.id} className="py-2 border-b border-gray-100 last:border-b-0">
                    <div className="flex items-center justify-between">
                      ...
                      <p className="font-semibold text-gray-900">{formatPrice(linePrice)}</p>
                    </div>
                    {opt && (
                      <p className="ml-8 text-xs text-indigo-600 mt-0.5">Option: {opt.name}</p>
                    )}
                  </div>
                );
              })}
      ...
      {selectedServices.length > 0 && (
        ...
        >
          <button
            onClick={() =>
              document
                .getElementById('selected-services-section')
                ?.scrollIntoView({ behavior: 'smooth', block: 'start' })
            }
