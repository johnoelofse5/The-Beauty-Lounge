...
export default function CreateAppointmentModal({
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (userRoleData?.role?.name === RoleName.Practitioner) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        isExternalClient &&
        (!externalClientInfo.firstName || !externalClientInfo.lastName || !externalClientInfo.phone)
      ) {
      ...
      try {
        await AppointmentSMSService.sendAppointmentNotifications(inserted.id, 'confirmation');
      } catch (err) {
        console.error('SMS error:', err);
      }
      try {
        await AppointmentCalendarService.createCalendarEvent(inserted.id);
      } catch (err) {
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {success ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setSelectedPractitioner(
                        practitioners.find((p) => p.id === e.target.value) ?? null
                      )
                    }
              ...
              {userRoleData?.role?.name === RoleName.Practitioner && selectedPractitioner && (
                <div>
                  ...
                  {[false, true].map((ext) => (
                    <label key={String(ext)} className="flex items-center">
                      ...
                      <span className="text-sm text-gray-700">
                        {ext ? 'New Client' : 'Existing Client'}
                      </span>
                    </label>
                  ))}
                </div>
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setExternalClientInfo((p) => ({ ...p, firstName: e.target.value }))
                          }
                          className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
                          required
                        />
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setExternalClientInfo((p) => ({ ...p, lastName: e.target.value }))
                          }
                          className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
                          required
                        />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setExternalClientInfo((p) => ({ ...p, phone: e.target.value }))
                        }
                        className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
                        required
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setExternalClientInfo((p) => ({ ...p, email: e.target.value }))
                        }
                        className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
                      />
                    </div>
