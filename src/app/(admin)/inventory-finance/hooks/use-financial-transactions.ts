...
export function useFinancialTransactions(
  ...
  const [isScannerModalClosing, setIsScannerModalClosing] = useState(false);
  ...
  const closeScannerModal = () => {
    setIsScannerModalClosing(true);
    setTimeout(() => {
      setShowScannerModal(false);
      setIsScannerModalClosing(false);
    }, 300);
  };
  const handleScanConfirm = (prefill: Partial<FinancialTransactionForm>) => {
    closeScannerModal();
    setTimeout(() => {
      setAddForm({ ...EMPTY_FORM, ...prefill });
      setAddErrors({});
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingTransaction || !validate(editForm, setEditErrors)) {
      showError('Please fix the errors in the form');
      return;
    }
    try {
  ...
  const handleMonthlyReport = async () => {
    try {
      ...
      if (error || !data?.success) {
        showError(data?.message || 'Failed to generate monthly report');
        return;
      }
      await downloadPDF(
        data.data.pdf_url,
        `monthly-financial-report-${yr}-${String(mo + 1).padStart(2, '0')}.pdf`
      );
  ...
  const handleYearlyReport = async () => {
    try {
      ...
      const dateTo = isNow
        ? currentDate.toISOString().split('T')[0]
        : new Date(yr, 11, 31).toISOString().split('T')[0];
      ...
      if (error || !data?.success) {
        showError(data?.message || 'Failed to generate yearly report');
        return;
      }
