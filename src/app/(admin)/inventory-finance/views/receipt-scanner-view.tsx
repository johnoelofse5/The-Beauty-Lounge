...
import BottomSheetModal from '../modals/bottom-sheet-modal';
import { ScanState } from '../types/scan-state';
import { ReceiptScannerViewProps } from '../types/receipt-scanner-view-props';
...
export function ReceiptScannerView({
    ...
    <>
      {scanState === ScanState.Camera && (
        <>
          ...
          <button
            onClick={capture}
            className="flex-1 bg-indigo-600 text-white px-4 py-3 rounded-lg font-medium hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2"
          >
      ...
      {scanState === ScanState.Captured && (
        <>
          ...
          <button
            onClick={runOCR}
            className="flex-1 bg-indigo-600 text-white px-4 py-3 rounded-lg font-medium hover:bg-indigo-700 transition-colors"
          >
            Scan Receipt
          </button>
        </>
  ...
  return (
    <BottomSheetModal title="Scan Receipt" isClosing={isClosing} onClose={onClose} footer={footer}>
      <div className="space-y-4">
        {scanState === ScanState.Camera && (
          <>
            {cameraError ? (
              ...
              <div className="relative bg-black rounded-xl overflow-hidden aspect-video max-h-48 sm:max-h-none">
                <video
                  ref={videoRef}
                  autoPlay
                  playsInline
        ...
        {scanState === ScanState.Upload && (
          <label className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:bg-gray-50 transition-colors">
            ...
            <span className="text-sm font-medium text-gray-700">Tap to select receipt image</span>
