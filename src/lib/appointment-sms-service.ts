...
export class AppointmentSMSService {
  ...
  private static async isSMSEnabled(smsType: SMSType): Promise<boolean> {
    ...
    try {
      ...
      if (error) {
        console.warn(
          "Could not read app_settings, defaulting to SMS enabled:",
          error,
        );
        return true;
      }
      ...
      console.warn(
        "Error checking SMS settings, defaulting to enabled:",
        error,
      );
      return true;
    }
  ...
  private static async log(
    ...
    smsType: SMSType,
    status: "sent" | "suppressed" | "failed" | "scheduled",
    ...
    if (error) console.error("Error writing SMS log:", error);
  }
  //#endregion
  ...
  static async sendAppointmentNotifications(
    ...
    try {
      ...
      if (!enabled) {
        await this.log(appointmentId, smsType, 'suppressed', {
          reason: `"${smsType}" disabled in app settings`,
        })
        return {
          success: true,
      ...
      if (error) {
        await this.log(appointmentId, smsType, 'failed', {
      ...
      await this.log(appointmentId, smsType, 'sent', {
        client_sms_sent: data.data?.client_sms_sent ?? false,
        practitioner_sms_sent: data.data?.practitioner_sms_sent ?? false,
        scheduled: data.data?.scheduled ?? false,
        schedule_date: data.data?.schedule_date ?? null,
      })
  ...
  static async scheduleReminderSMS(appointmentId: string) {
    try {
      ...
      return {
        success: true,
        message: "Reminder scheduled for daily processing",
      };
    } catch (error) {
