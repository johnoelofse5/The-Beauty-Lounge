...
export interface BookingProgress {
  ...
  selected_service_options?: Record<string, string>;
...
export class BookingProgressService {
  static async saveProgress(
    userId: string,
    progress: Partial<BookingProgress>
  ): Promise<BookingProgress | null> {
    try {
      ...
      const existingProgress =
        existingProgressData && existingProgressData.length > 0 ? existingProgressData[0] : null;

      const progressData = {
        user_id: userId,
        current_step: progress.current_step || 1,
        selected_services: progress.selected_services
          ? JSON.stringify(progress.selected_services)
          : null,
        selected_service_options: progress.selected_service_options
          ? JSON.stringify(progress.selected_service_options)
          : null,
        ...
        external_client_info: progress.external_client_info
          ? JSON.stringify(progress.external_client_info)
          : null,
  ...
  private static parseProgress(data: any): BookingProgress {
    return {
      ...
      selected_service_options: data.selected_service_options
        ? JSON.parse(data.selected_service_options)
        : undefined,
      ...
      external_client_info: data.external_client_info
        ? JSON.parse(data.external_client_info)
        : undefined,
