-- Comprehensive script to assign super_admin role to johnoelofse5@gmail.com
...
DECLARE
    ...
    user_id uuid;
BEGIN
    SELECT EXISTS(SELECT 1 FROM users WHERE email = 'johnoelofse5@gmail.com') INTO user_exists;
    
    IF NOT user_exists THEN
        RAISE NOTICE 'User with email johnoelofse5@gmail.com does not exist.';
        RAISE NOTICE 'Please create the user account first through the application.';
        RETURN;
    END IF;
    
    -- Get user ID
    SELECT id INTO user_id FROM users WHERE email = 'johnoelofse5@gmail.com';
...
BEGIN
    ...
    IF NOT role_exists THEN
        RAISE NOTICE 'Super admin role does not exist. Please run the roles migration first.';
        RETURN;
    END IF;
    
    -- Get role ID
    SELECT id INTO role_id FROM roles WHERE name = 'super_admin';
    RAISE NOTICE 'Found super_admin role with ID: %', role_id;
END $$;
...
-- Step 5: Update user to super_admin role
UPDATE users 
SET 
...
-- Step 7: Show all super_admin permissions
SELECT 
...
DO $$
BEGIN
    RAISE NOTICE '✅ Successfully assigned super_admin role to Stacey.mattheys@gmail.com!';
    RAISE NOTICE 'The user now has full administrative privileges.';
    RAISE NOTICE 'You can now access all admin features in the application.';
END $$;
