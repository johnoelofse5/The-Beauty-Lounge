...
serve(async (req) => {
  ...
  try {
    ...
    if (appointmentError || !appointment) {
      ...
      return new Response(JSON.stringify({ success: false, message: 'Appointment not found' }), {
        status: 404,
    ...
    if (servicesError) {
      ...
      return new Response(
        JSON.stringify({ success: false, message: 'Failed to fetch service details' }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    const practitionerEmail = practitionerProfile?.email || '';
    ...
    if (practitionerName) eventDescription += `Practitioner: ${practitionerName}\n`;
    eventDescription += `Services: ${serviceNames}\n`;
    if (appointment.notes) eventDescription += `\nNotes: ${appointment.notes}\n`;
    ...
    if (practitionerEmail) attendees.push({ email: practitionerEmail });
    if (attendees.length > 0) calendarEvent.attendees = attendees;
    ...
    return new Response(
      JSON.stringify({
        ...
        data: { event_id: eventId, calendar_url: eventHtmlLink },
      }),
