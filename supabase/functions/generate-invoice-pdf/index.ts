...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      try {
        await (supabase as any).storage.createBucket('branding', { public: true });
      } catch (_e) {}
      const logoPath = 'the beauty lounge logo.png';
      const { data: logoUrlData } = (supabase as any).storage
        .from('branding')
    ...
    draw(
      `Date: ${invoice.invoice_date ? new Date(invoice.invoice_date).toLocaleDateString('en-ZA') : new Date().toLocaleDateString('en-ZA')}`,
      50,
      765
    );
    ...
    draw(
      `Date: ${invoice.appointment_date ? new Date(invoice.appointment_date).toLocaleDateString('en-ZA') : 'N/A'}`,
      350,
      720
    );
    ...
    draw(
      `Total: R${(Number(invoice.total_amount || 0) || computedTotal).toFixed(2)}`,
      50,
      y - 28,
      14,
      true
...
async function generateInvoiceForAppointment(
  supabase: any,
  ...
  if (appointment.is_external_client) {
    ...
    const { data: authUser, error: clientError } = await supabase.auth.admin.getUserById(
      appointment.user_id
    );

    if (clientError || !authUser?.user) {
  ...
  const { data: optionRows, error: optionsFetchError } = await supabase
  ...
  if (optionsFetchError) {
    console.error('Service options fetch error:', optionsFetchError);
    throw new Error('Failed to fetch service options');
  }

  const optionLineItems: InvoiceLineItem[] = (optionRows || [])
    .map((row: any) => row.service_options)
    .filter(Boolean)
    .map((opt: any) => ({
      id: opt.id,
      name: opt.name,
      price: Number(opt.price_adjustment || 0),
      duration_minutes: opt.duration_adjustment_minutes || 0,
    }));

  console.log('Service options:', optionLineItems);

  const allLineItems = [...servicesData, ...optionLineItems];

  const totalAmount = allLineItems.reduce((sum, item) => sum + item.price, 0);
  ...
  const { data: invoice, error: invoiceError } = await supabase
    ...
    .select(
      'id, invoice_number, client_name, total_amount, client_email, services_data, appointment_date, invoice_date'
    )
    .single();
