...
serve(async (req) => {
  ...
  try {
    ...
    const practitionerName =
      `${appointment.practitioners?.first_name || ''} ${appointment.practitioners?.last_name || ''}`.trim();
    ...
    const calendarResponse = await fetch(
      `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(googleCalendarId)}/events/${appointment.google_calendar_event_id}?sendUpdates=${sendInviteEmail ? 'all' : 'none'}`,
      {
        method: 'PUT',
        headers: {
